...
public interface MessageRepository extends JpaRepository<Message, Integer> {
    @Query("SELECT new vn.edu.nlu.edushare.edu_share.api.chat.dto.ChatMessageDto(" +
            "m.id, m.conversation.id, m.senderId, m.content, m.isRead, m.createdAt) " +
            "FROM Message m " +
            "WHERE m.conversation.id = :conversationId " +
            "ORDER BY m.createdAt ASC")
    List<ChatMessageDto> findByConversationIdOrderByCreatedAtAsc(@Param("conversationId") Integer conversationId);
}
